...
# Training loop
def finetuning_sam(net, dataloader, optimizer, lr_scheduler, criterion, testloaders, epochs, device, args):
    loss_hist = []
    try:
    ...
    for epoch in range(epochs):
        ...
        if args.wandb is not None:  # Log results with wandb
            args.wandb.log(
                {
                    f'SAM Finetuning (SGD - {args.lr}) Training Loss': loss,
                    f'SAM Finetuning (SGD - {args.lr}) Clean Accuracy': clean_acc,
                    f'SAM Finetuning (SGD - {args.lr}) Attack Success Rate': poison_acc,
                }, 
                # step=epoch,
            )
        ...
        if epoch % args.p_intervals == 0:
            ...
            print_and_log(args.logger, f'SAM Fine-tuning epoch {epoch} Clean Accuracy: {clean_acc}')
            print_and_log(args.logger, f'SAM Fine-tuning epoch {epoch} Poison Accuracy: {poison_acc}')
...
def finetune_epoch(net, dataloader, optimizer, lr_scheduler, criterion, epoch, device, args):
    ...
    avg_loss = 0
    count = 0
    # pbar = tqdm.tqdm(dataloader, desc=f'SAM Finetuning Epoch: {epoch}')
